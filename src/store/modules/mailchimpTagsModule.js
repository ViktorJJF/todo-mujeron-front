...
import api from "@/services/api/mailchimpTags";
import {
  buildSuccess,
  handleError,
...
const module = {
  ...
  mutations: {
    ...
    create(state, data) {
      state.mailchimpTags.unshift(data);
    },
    update(state, { id, data }) {
      let indexToUpdate = state.mailchimpTags.findIndex(
        (member) => member._id == id
      );
      state.mailchimpTags.splice(indexToUpdate, 1, {
        ...data,
      });
    },
    delete(state, id) {
      let indexToDelete = state.mailchimpTags.findIndex(
        (member) => member._id == id
      );
      state.mailchimpTags.splice(indexToDelete, 1);
    },
  },
  getters: {},
};
