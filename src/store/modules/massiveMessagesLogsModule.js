...
import api from "@/services/api/massiveMessagesLogs";
...
const module = {
  ...
  mutations: {
    list(state, data) {
      state.massiveMessagesLogs = data;
    },
    create(state, data) {
      state.massiveMessagesLogs.unshift(data);
    },
    update(state, { id, data }) {
      let indexToUpdate = state.massiveMessagesLogs.findIndex(
      ...
      state.massiveMessagesLogs.splice(indexToUpdate, 1, {
        ...data,
      });
    },
    delete(state, id) {
      let indexToDelete = state.massiveMessagesLogs.findIndex(
        (member) => member._id == id
      );
      state.massiveMessagesLogs.splice(indexToDelete, 1);
    },
  },
  getters: {
    getLabelsByCategoryId: (state) => (categoryId) => {
      return state.massiveMessagesLogs.filter(
