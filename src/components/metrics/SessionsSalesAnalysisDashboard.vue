<template>
  <div class="sessions-sales-analysis-dashboard">
    <unified-date-filter
      title="Análisis de Ventas - Sesiones"
    ...
    <template v-else-if="viewMode === VIEW_MODES.COUNTS && dashboardData">
      ...
      <unified-summary-card
        ...
        :stats="[
          {
            value: dashboardData.totalSessionsWithSalesAnalysis || 0,
      ...
      <!-- Metrics Charts -->
      <v-row v-if="dashboardData.metrics">
        <v-col
          cols="12"
        ...
        >
          <unified-bar-chart
            :data="metric.data"
            :total="metric.total"
            :title="formatFieldTitle(metric.category)"
            icon="mdi-chart-bar"
            :horizontal="true"
          />
        </v-col>
    ...
    <template v-else-if="viewMode === VIEW_MODES.MONTHLY && monthlyData">
      ...
      <unified-summary-card
        title="Resumen de Tendencias Mensuales - Análisis de Ventas"
      ...
      <v-col cols="12" class="text-center">
        <v-card class="pa-6 empty-state-card">
          ...
          <div class="text-h6 grey--text text--darken-1 mt-3">
            Seleccione un rango de fechas para ver el análisis de ventas de
            sesiones
          </div>
        </v-card>
      </v-col>
...
export default {
  name: "SessionsSalesAnalysisDashboard",
  ...
  computed: {
    ...
    monthlyLabels() {
      ...
      return this.monthlyData.monthlyData.map(
        (month) =>
          `${month.monthName} ${month.year} (${month.totalSessionsWithSalesAnalysis || 0} sesiones)`
      );
    },
    ...
    totalMonthlyChats() {
      ...
      return this.monthlyData.monthlyData.reduce(
        (total, month) => total + (month.totalSessionsWithSalesAnalysis || 0),
        0
      );
    },
    ...
    groupedMonthlyDatasets() {
      ...
      if (this.monthlyData.monthlyData.length > 0) {
        const firstMonth = this.monthlyData.monthlyData[0];
        if (firstMonth.metrics) {
          firstMonth.metrics.forEach((metric) => {
            grouped[metric.category] = this.createDatasetsForCategory(metric.category);
          });
        }
      }
  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        ...
        if (this.viewMode === VIEW_MODES.COUNTS) {
          const response =
            await metricsApi.getSessionsSalesAnalysisCounts(params);
          this.dashboardData = response.data.payload;
        } else {
          const response =
            await metricsApi.getSessionsSalesAnalysisByMonth(params);
          this.monthlyData = response.data.payload;
        }
      } catch (error) {
        console.error("Error fetching sessions sales analysis data:", error);
        this.$toast.error(
          "Error al cargar los datos de análisis de ventas de sesiones"
        );
      } finally {
        this.loading = false;
      }
    ...
    createDatasetsForCategory(categoryName) {
      ...
      this.monthlyData.monthlyData.forEach((month) => {
        const metric = month.metrics?.find((m) => m.category === categoryName);
