<template>
  <div class="sessions-consultation-motives-dashboard">
    <unified-date-filter
      title="Análisis de Motivos de Consulta - Sesiones"
    ...
    <template v-else-if="viewMode === VIEW_MODES.COUNTS && dashboardData">
      ...
      <unified-summary-card
        title="Resumen de Sesiones con Motivos de Consulta"
        icon="mdi-chat-processing-outline"
        color="primary"
        :stats="[
          {
            value: dashboardData.totalSessionsWithMotives || 0,
    ...
    <template v-else-if="viewMode === VIEW_MODES.MONTHLY && monthlyData">
      ...
      <unified-summary-card
        title="Resumen de Tendencias Mensuales - Motivos de Consulta"
...
export default {
  name: "SessionsConsultationMotivesDashboard",
  ...
  computed: {
    ...
    monthlyLabels() {
      ...
      return this.monthlyData.monthlyData.map(
        (month) =>
          `${month.monthName} ${month.year} (${month.totalSessionsWithMotives || 0} sesiones)`
      );
    },
    ...
    totalMonthlyChats() {
      ...
      return this.monthlyData.monthlyData.reduce(
        (total, month) => total + (month.totalSessionsWithMotives || 0),
        0
      );
    },
    ...
    groupedMonthlyDatasets() {
      ...
      // Get all unique categories from the first month
      if (this.monthlyData.monthlyData.length > 0) {
        const firstMonth = this.monthlyData.monthlyData[0];
        if (firstMonth.arrayCategories) {
          firstMonth.arrayCategories.forEach((cat) => {
            grouped[cat.category] = this.createDatasetsForCategory(
              cat.category
            );
          });
        }
  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        ...
        if (this.viewMode === VIEW_MODES.COUNTS) {
          const response =
            await metricsApi.getSessionsConsultationMotivesCounts(params);
          this.dashboardData = response.data.payload;
        } else {
          const response =
            await metricsApi.getSessionsConsultationMotivesByMonth(params);
          this.monthlyData = response.data.payload;
        }
      } catch (error) {
        console.error("Error fetching sessions consultation motives data:", error);
        this.$toast.error(
          "Error al cargar los datos de motivos de consulta de sesiones"
        );
      } finally {
        this.loading = false;
      }
    ...
    createDatasetsForCategory(categoryName) {
      if (!this.monthlyData || !this.monthlyData.monthlyData) return [];

      const datasets = [];
      const valueLabels = new Set();

      // Collect all unique values for this category
      this.monthlyData.monthlyData.forEach((month) => {
        const category = month.arrayCategories?.find(
      ...
      Array.from(valueLabels).forEach((valueLabel, index) => {
        const data = this.monthlyData.monthlyData.map((month) => {
          ...
          if (category && category.data) {
            const item = category.data.find((d) => d.value === valueLabel);
